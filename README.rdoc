...
Gbooks is a lightweight wrapper for Google Book Search on Rails. It is inspired by the google-book gem which utilizes the deprecated Google Books API, but is updated to hook into the current Google API.

Though Gbooks provides access to the majority of relevant attributes provided by Google Books, it is not entirely comprehensive.

By default, Gbooks can query the following attributes:
...
Gbooks can circumvent these issues by adding an X-Forwarded-For HTTP header to each request that contains the IP address of the user. In most cases, you can pass the user's IP address as a parameter in your search request:

  Gbooks.search("Search Query", {options}, request.remote_ip)
...
Gbook can be configured to an return either ISBN, but attempts to return the 13-digit version by default. If the 13-digit version cannot be found, Gbooks tries the 10-digit version, and finally returns nil.

=== Image links

When possible, Gbooks will return links to an image of a book's cover. The size of this image is configurable by passing an numerical argument between 1-7 to the image_link attribute, which is passed to the zoom attribute in the URL query.

  book = Gbooks.search("The Great Gatsby").first
...
This enables you to manipulate and cull different sized images on the fly without cluttering the codebase with added methods.
