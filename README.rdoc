...
The current Google API for Books requires generally that a users IP address be geolocatable. This can cause responses to be restricted when requested from distributed/clustered servers such as Heroku.

http://www.google.com/support/forum/p/booksearch-apis/thread?tid=2034bed9a98c15cb&hl=en

Gbooks can circumvent these issues by adding an X-Forwarded-For HTTP header to each request that contains the IP address of the user. In most cases, you can pass the user's IP address in your search request:
